...
class _BitcoinScreenState extends State<BitcoinScreen>
  ...
  final List<String> ranges = ['10m', '30m', '1h', '4h', '24h', '7d', '1m'];

  List<double> anomalies = [];
  String formatNumber(double value) {
    final NumberFormat formatter = NumberFormat(
      '#0.000',
    ); // 3 ondalıklı gösterim
  ...
  void initState() {
    ...
    _timer = Timer.periodic(Duration(seconds: 10), (Timer t) {
      if (!isLoading) {
        // Yükleme sırasında tekrar çağırmaktan kaçının
        _fetchChartData();
      }
    });
  ...
  Future<void> _fetchChartData() async {
    ...
    try {
      Map<String, List<FlSpot>> data = await fetchCryptoChartData(
        selectedRange,
      );
      List<double> prices =
          data[selectedRange]?.map((spot) => spot.y).toList() ?? [];
      ...
      setState(() {
        ...
        anomalies =
            detectedAnomalies.cast<double>(); // Anomalileri güncelliyoruz
  ...
  List<double> detectAnomaliesWithZScore(
    List<double> prices,
    double threshold,
  ) {
  ...
  List<double> detectAnomaliesWithSlidingWindow(
    List<double> prices,
    int windowSize,
  ) {
    List<double> anomalies = [];
    for (int i = windowSize; i < prices.length; i++) {
      ...
      double localVariance =
          window.map((p) => pow(p - localMean, 2)).reduce((a, b) => a + b) /
          window.length;
  ...
  List<double> detectCommonAnomalies(
    List<double> prices, {
    double zScoreThreshold = 2.5,
    int slidingWindowSize = 5,
  }) {
    // Z-Score yöntemi
    List<double> anomaliesZScore = detectAnomaliesWithZScore(
      prices,
      zScoreThreshold,
    );
    ...
    List<double> anomaliesSlidingWindow = detectAnomaliesWithSlidingWindow(
      prices,
      slidingWindowSize,
    );
            ...
            icon:
                isLoading
                    ? CircularProgressIndicator(
                      color: Colors.white,
                    ) // Yüklenirken dönen animasyon
                    : Icon(Icons.refresh, color: Colors.white),
            onPressed:
                isLoading
                    ? null
                    : _fetchChartData, // Yükleme sırasında buton devre dışı
          ),
        ],
      ),
      body: Padding(
        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              Text(
                change != null
                    ? '${change!.toStringAsFixed(2)} (${((change! / lastPrice!) * 100).toStringAsFixed(2)}%)'
                    : '',
                style: TextStyle(
                  color:
              ...
              TabBar(
                ...
                tabs:
                    ranges
                        .map((range) => Tab(text: range.toUpperCase()))
                        .toList(),
              ),
              SizedBox(
                      ...
                      child: LineChart(
                        LineChartData(
                          ...
                          lineBarsData: [
                            ...
                            LineChartBarData(
                              spots:
                                  chartData[selectedRange]!.map((spot) {
                                    final formattedY = double.parse(
                                      formatNumber(spot.y),
                                    );
                            ...
                            LineChartBarData(
                              spots:
                                  anomalies.map((anomaly) {
                                    ...
                                    final spot = chartData[selectedRange]!
                                        .firstWhere(
                                          (s) => s.y == anomaly,
                              ...
                              color:
                                  Colors
                                      .red, // Bu renk parametresi doğru kullanıldı.
                              dotData: FlDotData(
                                show: true,
                              ), // dotSize ve colors parametreleri burada kullanılmıyor
  ...
  List<FlSpot> calculateRSI(List<FlSpot> data) {
    ...
    for (int i = 0; i < gains.length; i++) {
      ...
      double avgGain =
          gains.sublist(i - period + 1, i + 1).reduce((a, b) => a + b) / period;
      double avgLoss =
          losses.sublist(i - period + 1, i + 1).reduce((a, b) => a + b) /
          period;
  ...
  List<List<FlSpot>> calculateBollingerBands(List<FlSpot> data) {
    ...
    for (int i = 0; i < data.length; i++) {
      ...
      List<double> subset =
          data.sublist(i - period + 1, i + 1).map((spot) => spot.y).toList();
