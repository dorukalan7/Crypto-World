...
class _FullScreenChartState extends State<FullScreenChart> {
  ...
  Future<void> fetchPrediction() async {
    ...
    try {
      final response = await http.get(
        Uri.parse("https://7d6b-34-16-177-8.ngrok-free.app/get_bitcoin_data"),
      );
      ...
      if (response.statusCode == 200) {
        ...
        for (int i = 0; i < predictionValues.length; i++) {
          ...
          double xValue =
              predictionDate.millisecondsSinceEpoch /
              1000.0; // Unix timestamp formatında X değeri
            ...
            child: InteractiveViewer(
              ...
              child: SingleChildScrollView(
                ...
                child: SizedBox(
                  ...
                  width:
                      (widget.data.length + predictionData.length) *
                      10.0, // Genişlik daha fazla olabilir
                  height: 90,
                  child: LineChart(
                    LineChartData(
                      ...
                      titlesData: FlTitlesData(
                        bottomTitles: AxisTitles(
                          sideTitles: SideTitles(
                            ...
                            reservedSize: 22,
                            interval:
                                10, // Bu aralık değeri veri yoğunluğuna göre ayarlanabilir
                            getTitlesWidget: (value, meta) {
                              DateTime date =
                                  DateTime.fromMillisecondsSinceEpoch(
                                    (value * 1000).toInt(),
                                  );
                              DateTime oneMonthLater = date.add(
  ...
  List<FlSpot> convertToFlSpots(Map<String, dynamic> data) {
    ...
    for (int i = 0; i < predictionData.length; i++) {
      ...
      double yValue =
          predictionData[i].toDouble(); // Y değeri olarak tahmin kullanılıyor
      spots.add(FlSpot(xValue, yValue));
    }
    return spots;
  }
